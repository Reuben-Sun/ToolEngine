...
namespace ToolEngine
{
	...
	DescriptorSetLayout::DescriptorSetLayout(Device& device): m_device(device)
	{
		...
		// per mesh ubo
		VkDescriptorSetLayoutBinding per_mesh_ubo_layout_binding{};
		per_mesh_ubo_layout_binding.binding = 1;
		per_mesh_ubo_layout_binding.descriptorCount = 1;
		per_mesh_ubo_layout_binding.descriptorType = DescriptorTypeMap[DescriptorType::DynamicBuffer];
		per_mesh_ubo_layout_binding.pImmutableSamplers = nullptr;
		per_mesh_ubo_layout_binding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;

		// create info
		std::vector<VkDescriptorSetLayoutBinding> bindings = { ubo_layout_binding, per_mesh_ubo_layout_binding };
